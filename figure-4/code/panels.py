...
from utils import panels_file_path
...
class Panel:
    ...
    def from_dict(cls, dictionary):
        ...
        inform_phenotypes = dictionary["inform_phenotypes"]
    ...
    @property
    def foreground_subtypes(self):
        if self.__foreground_subtypes is None:
            self.__foreground_subtypes = sorted(
                list(set([pheno.subtype for pheno in self.phenotypes if not pheno.background])))
        return self.__foreground_subtypes
    ...
    def prediction_phenotype(self, distance, predicted_pheno, activ_th=0.4, detection_th=3.5, mm_pp=2):
        ...
        if distance > detection_th * mm_pp:
            return SpecialPhenotypes.no.make_anntype()
    ...
    def inform_phenotype(
            self, distance, pred_type, detection_th=3.5, mm_pp=2
    ):
        if distance > detection_th * mm_pp:
            return SpecialPhenotypes.other.make_anntype()

        subtype = self.inform_phenotypes[pred_type]
        match_phenotypes = [pheno for pheno in self.phenotypes if pheno.subtype == subtype]

        if len(match_phenotypes) == 0 or len(match_phenotypes) > 1:
            return SpecialPhenotypes.other.make_anntype()

        return AnnotationType(match_phenotypes[0].main_type, match_phenotypes[0].subtype)


def load_panels(relative_path=panels_file_path):
    panels = {}
